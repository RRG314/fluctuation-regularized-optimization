...
# ---------------------------------------------------------------------------
# CasimirSwarm integration
# ---------------------------------------------------------------------------

def test_swarm_float32_dtype_runs():
    swarm = CasimirSwarm([(-3, 3)] * 3, n_particles=12, dtype=torch.float32,
                         seed=0)
...
def test_swarm_one_dimensional_problem():
    swarm = CasimirSwarm([(-10.0, 10.0)], n_particles=16, seed=1)
...
def test_swarm_callback_contract():
    seen = []
    swarm = CasimirSwarm([(-1, 1)] * 2, n_particles=8, seed=2)
...
def test_swarm_boltzmann_reflectivity_mode():
    swarm = CasimirSwarm([(-2, 2)] * 2, n_particles=12,
...
def test_swarm_unknown_reflectivity_raises():
    swarm = CasimirSwarm([(-1, 1)], reflectivity="nope", seed=0)
...
def test_swarm_asymmetric_bounds_respected():
    lo, hi = 2.0, 9.0
    swarm = CasimirSwarm([(lo, hi)] * 3, n_particles=10, seed=6)
    box = []
...
def test_optimizer_multiple_param_groups():
    ...
    opt = CasimirOptimizer([{"params": [a], "lr": 5e-2},
...
def test_optimizer_weight_decay_shrinks_solution():
    ...
    opt = CasimirOptimizer([theta], lr=5e-2, sigma=1e-3, weight_decay=1.0,
                           seed=0)
    for _ in range(400):
        opt.step(closure)
    target = torch.arange(6.0)
...
def test_optimizer_odd_probe_count():
    theta, closure = _quadratic_problem()
    opt = CasimirOptimizer([theta], lr=5e-2, sigma=1e-3, n_probes=3, seed=0)
    for _ in range(500):
...
def test_optimizer_state_dict_roundtrip():
    theta, closure = _quadratic_problem()
    opt = CasimirOptimizer([theta], lr=1e-2, sigma=1e-3, seed=0)
    for _ in range(5):
        opt.step(closure)
    sd = opt.state_dict()
    opt2 = CasimirOptimizer([theta], lr=1e-2, sigma=1e-3, seed=0)
...
def test_optimizer_frozen_params_untouched():
    ...
    opt = CasimirOptimizer([a, frozen], lr=5e-2, sigma=1e-3, seed=0)
    for _ in range(20):
...
def test_optimizer_restores_params_after_probing():
    ...
    opt = CasimirOptimizer([theta], lr=0.0, sigma=0.5, n_probes=4, seed=0)
    opt.step(closure)
...
def test_balancer_weight_clamps_respected():
    ...
    bal = CasimirPressureBalancer([p], n_terms=2, w_min=0.5, w_max=2.0,
                                  update_every=1)
...
def test_balancer_update_every_caches_weights():
    p = torch.nn.Parameter(torch.randn(4))
    bal = CasimirPressureBalancer([p], n_terms=2, update_every=10)
...
def test_end_to_end_tiny_pinn_smoke():
    """One coupled run of MLP + balancer + CasimirOptimizer on a 1D ODE:
    u' = u, u(0) = 1 on [0, 0.5]; just verify the machinery trains."""
    torch.manual_seed(0)
    net = MLP([1, 16, 1])
    bal = CasimirPressureBalancer(net.parameters(), n_terms=2, update_every=5)
    opt = CasimirOptimizer(net.parameters(), lr=2e-2, sigma=1e-3, seed=0)
